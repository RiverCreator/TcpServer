                  ...
                  acceptor_(mainloop_.get(),ip,port),threadpool_(threadnum_,"IO")
{
    ...
    {
        subloops_.emplace_back(new EventLoop(false,5));              // 创建从事件循环，存入subloops_容器中。
        ...
        subloops_[ii]->setclosecallback(std::bind(&TcpServer::closeconnection, this, std::placeholders::_1));
...
void TcpServer::closeconnection(spConnection conn)
{
    ...
    {
            std::lock_guard<std::mutex> gd(mmutex_);
            //printf("conn use count:%d\n",conns_[conn->fd()].use_count());
...
{
    //std::cout << "tcp server:" << message << std::endl;
    LOG_WAR("TcpServer onmessage");
    if (onmessagecb_)
        onmessagecb_(conn, message); // 回调上层业务类的HandleMessage()。
}
...
void TcpServer::removeconn(int fd)                 
{
    {
        std::lock_guard<std::mutex> gd(mmutex_);
        LOG_INFO<<"TcpServer:"<< conns_[fd].use_count();
