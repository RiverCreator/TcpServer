...
// 在构造函数中创建Epoll对象ep_。
EventLoop::EventLoop(bool mainloop,int timetvl):ep_(new Epoll),mainloop_(mainloop),
                   timetvl_(timetvl),stop_(false),
        ...
        for (auto it = conns_.begin(); it != conns_.end();)
        {
            ...
            if (it->second->timeout(now))
            {
                ...
                timercallback_(it->first); // 从TcpServer的map中删除超时的conn
                {
                    ...
                    //std::cout <<"eventloop:"<< it->second.use_count() << std::endl;
                    conns_.erase(it++); // 从EventLoop的map中删除超时的conn。
                }
            }
            else
                it++;
...
void EventLoop::newconnection(spConnection conn)
{
    std::lock_guard<std::mutex> gd(mmutex_);
    conns_[conn->fd()]=conn;
    conn->setclosecallback(std::bind(&EventLoop::handleclose,this,std::placeholders::_1));
}
...
void EventLoop::handleclose(spConnection conn){
    std::lock_guard<std::mutex> gd(mmutex_);
    conns_.erase(conn->fd());
    closecallback(conn); //调用TcpServer的closeconnection
}

void EventLoop::setclosecallback(std::function<void(spConnection)> fn){
    closecallback = fn;
}

void EventLoop::settimeout(int timeouts){
    struct itimerspec timeout;                                // 定时时间的数据结构。
    memset(&timeout,0,sizeof(struct itimerspec));
    timeout.it_value.tv_sec = timeouts;                             // 定时时间，固定为5，方便测试。
    timeout.it_value.tv_nsec = 0;
    timerfd_settime(timerfd_,0,&timeout,0);
    timeout_=timeouts;
}