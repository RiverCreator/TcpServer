...
private:
    ...
    std::function<void(spConnection)> closecallback;

public:
    EventLoop(bool mainloop=false,int timetvl=30);                   // 在构造函数中创建Epoll对象ep_。
    ~EventLoop();               
    ...
    void setclosecallback(std::function<void(spConnection)> fn); // 将被设置为TcpServer::removeconn()
